<?php 
require_once'header.php'; 
$title = "Get in contact with the organic farm in Redmond, OR | Dome Grown Produce";
$metad = "Dome Grown Produce wants to listen to what you have to say about the farm that has CSA share";
?>
				
			
					<div class="hero" id="hero">

						<img src="photos/stormy-night-in-redmond-local-farms.jpg" alt="Local produce for CSA shares in Redmond, OR">
						
					</div>
				
<?php require_once'cta.php'; ?>			

		</header>
				
				
					<div class="row1-content cf">
						<div class="left-65 left-row-1">
								<h1><span>Contact Dome Grown Produce</span></h1>
								

							<p>Check back soon to be able to contact Dome Grown Produce.
							</p>
							

					
						</div>
								<div class="right-35 right-row-1 center ">
									
									
									
								</div>

					</div>
				

							


			
<?php require_once'footer.php' ?>
					

	</div>
</body>
</html>